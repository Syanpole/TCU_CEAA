@echo off
REM Quick fix for CI dependency installation issues
REM This script installs dependencies in a way that avoids compilation problems
...
if %errorlevel% neq 0 pip install opencv-python-headless
...
if %errorlevel% neq 0 echo ⚠️ matplotlib installation failed (optional)
pip install --only-binary=all scipy
if %errorlevel% neq 0 echo ⚠️ scipy installation failed (optional)

echo ✅ Installation complete!
echo 🔍 Checking installed packages...
pip list | findstr /i "Django numpy opencv scikit nltk PyPDF2"
